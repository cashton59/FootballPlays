...
function paintField(ctx) {
  ...
  const YARD=36, fs=52;
  for (let i=0;;i++) {
    const y=fs+i*YARD; if(y>CH) break;
    if(i%2===0){ctx.fillStyle="rgba(0,0,0,0.04)";ctx.fillRect(0,y,CW,Math.min(YARD,CH-y));}
  }
  ctx.fillStyle="#e0e0e0"; ctx.fillRect(0,0,CW,fs);
  ctx.strokeStyle="rgba(0,0,0,0.55)"; ctx.lineWidth=1.5; ctx.setLineDash([]);
  ctx.beginPath(); ctx.moveTo(0,fs); ctx.lineTo(CW,fs); ctx.stroke();
  ctx.strokeStyle="rgba(0,0,0,0.15)"; ctx.lineWidth=1;
  for(let y=fs;y<=CH;y+=YARD){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(CW,y);ctx.stroke();}
  ...
  const losY=CH-130;
  ctx.save(); ctx.strokeStyle="rgba(0,0,0,0.25)"; ctx.lineWidth=1; ctx.setLineDash([6,6]);
  ctx.beginPath();ctx.moveTo(4,losY);ctx.lineTo(CW-4,losY);ctx.stroke(); ctx.restore();
  ctx.fillStyle="rgba(0,0,0,0.22)"; ctx.font="bold 9px 'Courier New',monospace";
  ctx.fillText("L·O·S",8,losY-4);
  ctx.fillStyle="rgba(0,0,0,0.35)"; ctx.font="bold 12px 'Courier New',monospace";
  ctx.textAlign="center"; ctx.fillText("END ZONE",CW/2,31); ctx.textAlign="left";
}

function paintLine(ctx, line, alpha=1) {
  const {type, points, color, finisher="arrow"} = line;
  if(!points||points.length<2) return;
  ctx.save(); ctx.globalAlpha=alpha; ctx.strokeStyle=color||"#000";
  ctx.lineWidth=2.4; ctx.lineCap="round"; ctx.lineJoin="round";
  ctx.setLineDash(type==="dashed"?[8,5]:[]);
  const last=points[points.length-1]; let endAngle=0;
  if(type==="curved"){ctx.beginPath();endAngle=catmullRomPath(ctx,points);ctx.stroke();}
  else{const[a,b]=points;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();endAngle=Math.atan2(b.y-a.y,b.x-a.x);}
  ctx.setLineDash([]);
...
function paintPlayer(ctx, p, highlighted=false) {
  ...
  ctx.fillStyle=p.color; ctx.strokeStyle="rgba(0,0,0,0.7)"; ctx.lineWidth=2;
  if(p.shape==="circle"){ctx.beginPath();ctx.arc(p.x,p.y,PR,0,Math.PI*2);ctx.fill();ctx.stroke();}
  else{
    const s=PR*1.15,r=3;
    ctx.beginPath();
...
export default function FootballPlayCreator() {
  ...
  const [showPrint,   setShowPrint]   = useState(false);  // ← in-app print preview
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("fpc-plays");
  ...
  useEffect(() => {
    ...
    (async () => {
      try { await window.storage.set("fpc-plays", JSON.stringify(plays)); } catch(_) {}
    })();
  }, [plays, storageReady]);
  ...
  const cancelCurve = useCallback(() => {
    is.current.curvePoints=[]; is.current.curveActive=false; is.current.tempLine=null;
    setCurvePts(0); setStatusMsg(""); redraw();
  }, [redraw]);

  const commitCurve = useCallback(() => {
    const cp = is.current.curvePoints;
    if (cp.length<2) { cancelCurve(); return; }
    const nl={id:uid(),type:"curved",points:[...cp],color:is.current.lineColor,finisher:is.current.finisher};
    const ls2=[...is.current.lines,nl];
    is.current.lines=ls2; is.current.curvePoints=[]; is.current.curveActive=false; is.current.tempLine=null;
    setCurvePts(0); setLines(ls2); setStatusMsg(""); redraw();
  }, [cancelCurve, redraw]);
  ...
  const pushHistory = () => {
    history.current=[...history.current.slice(-24),{players:[...is.current.players],lines:[...is.current.lines]}];
  };
  const doUndo = () => {
    if(!history.current.length) return;
    const prev=history.current.pop();
    is.current.players=prev.players; is.current.lines=prev.lines;
    setPlayers(prev.players); setLines(prev.lines);
  };
  ...
  const playerAt = (x,y) => {
    ...
    for(let i=ps.length-1;i>=0;i--) if(Math.hypot(ps[i].x-x,ps[i].y-y)<=PR+5) return ps[i];
  };
  const lineAt = (x,y) => {
    ...
    for(const line of is.current.lines){
      ...
      if(line.type==="curved"&&pts.length>=2){
        ...
        const dx=pts[1].x-pts[0].x,dy=pts[1].y-pts[0].y,lenSq=dx*dx+dy*dy;
  ...
  const onMove = e => {
    ...
    if(s.tool==="erase"){
      const p=playerAt(pos.x,pos.y);
      s.eraseHoverId=p?p.id:(lineAt(pos.x,pos.y)?.id??null);redraw();return;
    }
    if(s.dragId){
      s.players=s.players.map(p=>p.id===s.dragId?{...p,x:pos.x-s.dragOx,y:pos.y-s.dragOy}:p);redraw();
    } else if((s.tool==="straight"||s.tool==="dashed")&&s.mouseDown&&s.drawStart){
      s.tempLine={type:s.tool,points:[s.drawStart,pos],color:s.lineColor,finisher:s.finisher};redraw();
    } else if(s.tool==="curved"&&s.curveActive&&s.curvePoints.length>=1){
      const prev=[...s.curvePoints,pos];
      s.tempLine=prev.length>=2?{type:"curved",points:prev,color:s.lineColor,finisher:s.finisher}
  ...
  const onUp = e => {
    ...
    if(s.dragId){
      ...
      if(d>8){pushHistory();const nl={id:uid(),type:s.tool,points:[s.drawStart,pos],color:s.lineColor,finisher:s.finisher};const ls2=[...s.lines,nl];s.lines=ls2;setLines(ls2);}
  ...
  const onDblClick = e => {
    const s=is.current;
    if(s.tool==="curved") return; // handled in onDown via e.detail
    if(s.tool==="select"){
  ...
  const savePlay = () => {
    ...
    const snapshot={players:JSON.parse(JSON.stringify(is.current.players)),lines:JSON.parse(JSON.stringify(is.current.lines))};
  ...
  const loadPlay = p => {
    ...
    setPlayers(is.current.players); setLines(is.current.lines);
    setPlayName(p.name+" (edit)"); history.current=[];
  };
  ...
  const openPrintWindow = () => {
    const ppp2 = parseInt(printLayout);
    const cols = ppp2 === 6 ? 3 : 2;
    const pages = [];
    for (let i = 0; i < plays.length; i += ppp2) pages.push(plays.slice(i, i + ppp2));
    const playsHTML = pages.map(group => {
      const cells = group.map(p => `
        <div class="cell">
          <div class="cell-title">${p.name.replace(/</g,"&lt;")}</div>
          <img src="${p.imgData}" alt="${p.name.replace(/"/g,"&quot;")}"/>
        </div>`).join("");
      return `<div class="page" style="grid-template-columns:repeat(${cols},1fr)">${cells}</div>`;
    }).join("");
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Play Book</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:#e8e8e8;font-family:Georgia,serif}
  .page{width:11in;height:8.5in;background:white;display:grid;grid-template-rows:repeat(2,1fr);gap:.15in;padding:.3in;margin:.2in auto;box-shadow:0 4px 20px rgba(0,0,0,.2);page-break-after:always;break-after:page}
  .cell{border:1.5px solid #333;border-radius:4px;overflow:hidden;display:flex;flex-direction:column}
  .cell-title{background:#222;color:white;font-weight:bold;font-size:10pt;text-align:center;padding:4px 8px;flex-shrink:0;letter-spacing:.5px}
  .cell img{flex:1;width:100%;display:block;object-fit:contain}
  .toolbar{background:#222;color:white;padding:10px 20px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:10}
  .toolbar button{background:#238636;color:white;border:none;padding:8px 20px;border-radius:6px;cursor:pointer;font-size:14px;font-weight:bold;margin-left:10px}
  @media print{body{background:white}.toolbar{display:none}.page{margin:0;box-shadow:none}@page{size:landscape;margin:0}}
</style></head><body>
<div class="toolbar">
  <span>🏈 Play Book — ${plays.length} play${plays.length!==1?"s":""} · ${pages.length} page${pages.length!==1?"s":""} · Landscape</span>
  <div>
    <button onclick="window.print()">🖨 Print</button>
    <button onclick="window.close()" style="background:#555;margin-left:8px">✕ Close</button>
  </div>
</div>
${playsHTML}
<script>window.onload=()=>window.print();<\/script>
  ...
  const switchTool = id => {
    ...
    is.current.eraseHoverId=null; setTool(id); redraw();
  };
  ...
  const toolTip={
    ...
    curved:is.current.curveActive?`${curvePts} pt${curvePts!==1?"s":""}  •  Dbl-click or Enter to finish`:"Click start → add bends → dbl-click finish",
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <aside style={{width:"172px",background:panel,borderRight:`1px solid ${bdr}`,padding:"14px 10px",display:"flex",flexDirection:"column",gap:"12px",flexShrink:0,overflowY:"auto"}}>
          ...
          {tool==="place"&&<section>
            ...
            <div style={{display:"flex",gap:"6px"}}>
              {[["circle","●"],["square","■"]].map(([s,ic])=>(
                <button key={s} onClick={()=>setShape(s)} style={{flex:1,padding:"7px 0",fontSize:"18px",background:shape===s?"#1f6feb":"transparent",color:shape===s?"white":tm,border:`1px solid ${shape===s?"#1f6feb":bdr}`,borderRadius:"6px",cursor:"pointer"}}>{ic}</button>
              ))}
            </div>
          </section>}
          ...
          {tool==="curved"&&is.current.curveActive&&(
            <section style={{background:"#192a19",borderRadius:"8px",padding:"10px",border:`1px solid ${accent}`}}>
              ...
              <button onClick={cancelCurve} style={{width:"100%",padding:"5px",background:"transparent",color:"#f85149",border:"1px solid #f85149",borderRadius:"5px",cursor:"pointer",fontSize:"11px",fontFamily:"inherit"}}>✕ Cancel</button>
            </section>
          )}
        ...
        <aside style={{width:"222px",background:panel,borderLeft:`1px solid ${bdr}`,display:"flex",flexDirection:"column",flexShrink:0}}>
          ...
          <div style={{padding:"12px 14px",borderTop:`1px solid ${bdr}`}}>
            ...
            <button onClick={openPrintWindow} disabled={plays.length===0}
            ...
            <div style={{display:"flex",gap:"6px",marginTop:"6px"}}>
              ...
              <button onClick={importPlays}
                style={{flex:1,padding:"7px 4px",background:"#192a19",color:"#4ade80",border:`1px solid #238636`,borderRadius:"6px",cursor:"pointer",fontWeight:"bold",fontSize:"11px",fontFamily:"inherit"}}>
                ⬆ IMPORT
              </button>
            </div>
            {plays.length>0&&<div style={{fontSize:"11px",color:tm,textAlign:"center",marginTop:"7px"}}>{plays.length} play{plays.length!==1?"s":""} · {Math.ceil(plays.length/ppp)} page{Math.ceil(plays.length/ppp)!==1?"s":""}</div>}
          </div>
        </aside>
      </div>
